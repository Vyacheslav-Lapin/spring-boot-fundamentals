...
public class AfterProxyCaller {//implements ApplicationListener<ContextRefreshedEvent> {
...
//    val applicationContext = (AnnotationConfigApplicationContext) event.getApplicationContext();

//    val tuple2s = Arrays.stream(beanFactory.getBeanDefinitionNames())
//        .map(beanName -> Tuple.of(beanName, applicationContext.getBeanDefinition(beanName)))
...
//        .filter(beanNameAndBeanMethod -> beanNameAndBeanMethod._2.isAnnotationPresent(AfterProxy.class))
